 ...
 *
 * @file virtio_block_frontend.h
 * @brief Implementation of virtio_block_frontend.h
...
// 设备扫描
uint64_t
scan_for_virtio_block_device(uint32_t device_id);
...
virtio_read32(virtio_device_t *dev, uint64_t offset)
{
    return mmio_read32((volatile void *) (dev->base_addr + offset));
}

static inline void
virtio_write32(virtio_device_t *dev, uint64_t offset, uint32_t value)
{
    mmio_write32(value, (volatile void *) (dev->base_addr + offset));
}

static inline uint64_t
virtio_read64(virtio_device_t *dev, uint64_t offset)
{
    return mmio_read64((volatile void *) (dev->base_addr + offset));
}

static inline void
virtio_set_status(virtio_device_t *dev, uint32_t status)
{
...
static inline uint32_t
virtio_get_status(virtio_device_t *dev)
{
