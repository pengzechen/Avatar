...
// 写屏障：确保之前写完成
static inline void mmio_write32(uint32_t value, volatile void *addr)
{
    dsb(sy);           // 确保前序写完成
    write32(value, addr);
...
// 同理封装 8/16/64 版本
static inline void mmio_write8(uint8_t value, volatile void *addr)
{
    dsb(sy);
    write8(value, addr);
    dsb(sy);
...
static inline void mmio_write16(uint16_t value, volatile void *addr)
{
    dsb(sy);
    write16(value, addr);
    dsb(sy);
...
static inline void mmio_write64(uint64_t value, volatile void *addr)
{
    dsb(sy);
    write64(value, addr);
    dsb(sy);
