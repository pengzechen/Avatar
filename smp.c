...
extern void _stack_bottom_second();
...
void show_stack_layout(void)
{
    ...
    if (SMP_NUM > 1) {
        logger("Secondary stack pool: [%llx - %llx] (%llu bytes)\n",
               (uint64_t)_stack_bottom_second, (uint64_t)_stack_top_second, total_secondary_stack);
...
void start_secondary_cpus()
{
    ...
    for (int32_t i = 1; i < SMP_NUM; i++)
    {
        ...
        // 栈边界检查 - 确保不会超出分配的栈空间
        uint64_t available_stack_space = (uint64_t)_stack_top_second - (uint64_t)_stack_bottom_second;
        ...
        if (required_stack_space > available_stack_space) {
            ...
            logger("       Required: %llu bytes, Available: %llu bytes\n",
                   required_stack_space, available_stack_space);
        ...
        logger("Starting CPU %d, stack: [%llx - %llx]\n",
               i, secondary_stack_bottom, secondary_stack_top);
        ...
        logger("CPU %d started successfully, thread info addr: %llx\n",
               i, (void *)(secondary_stack_top - STACK_SIZE));
    }
