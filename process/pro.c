...
static int32_t
load_phdr(const char *elf_file_addr, Elf64_Phdr *phdr, pte_t *page_dir)
{
    // 生成的ELF文件要求是页边界对齐的
...
static uint64_t
load_elf_file(process_t *pro, const char *elf_file_addr, pte_t *page_dir)
{
...
void
prepare_vm(process_t **process, void *elf_addr)
{
    ...
    pro->entry =
        load_elf_file(pro, elf_addr, (pte_t *) pro->pg_base);  // map data 区的一块内存 将来优化这里
    ...
    memory_create_map(pro->pg_base,
                      (uint64_t) pro->entry + 0x3000,
                      virt_to_phys(pro->el0_stack),
                      1,
                      0);
}
...
process_init(process_t *pro, void *elf_addr, uint32_t affinity)
{
    ...
    tcb_t *main_thread = create_task((void (*)())(void *) pro->entry,
                                     (uint64_t) pro->el1_stack + PAGE_SIZE * 2,
                                     affinity);
