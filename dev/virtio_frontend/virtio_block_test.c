 ...
 *
 * @file virtio_block_test.c
 * @brief Implementation of virtio_block_test.c
...
// 生成测试数据模式
static void
generate_test_pattern(uint8_t *buffer, uint32_t size, uint32_t seed)
{
...
// 验证测试数据模式
static bool
verify_test_pattern(const uint8_t *buffer, uint32_t size, uint32_t seed)
{
    for (uint32_t i = 0; i < size; i++) {
...
// 打印缓冲区内容（十六进制）
static void
print_hex_dump(const uint8_t *buffer, uint32_t size, const char *title)
{
    logger_info("%s:\n", title);
...
// 基本读取测试
int
virtio_blk_test_basic_read(virtio_blk_device_t *blk_dev)
{
...
// 读写测试
int
virtio_blk_test_read_write(virtio_blk_device_t *blk_dev)
{
...
// 性能测试
int
virtio_blk_test_performance(virtio_blk_device_t *blk_dev)
{
    ...
    for (uint32_t i = 0; i < iterations; i++) {
        ...
        for (uint32_t j = 0; j < test_sectors; j++) {
            if (virtio_blk_write_sector(blk_dev, test_start_sector + j, test_buffer + j * 512, 1) <
                0) {
        ...
        for (uint32_t j = 0; j < test_sectors; j++) {
            if (virtio_blk_read_sector(blk_dev, test_start_sector + j, test_buffer + j * 512, 1) <
                0) {
...
// 简单的设备信息查询
void
virtio_block_query_device(void)
{
