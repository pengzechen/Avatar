...
static fat32_cache_block_t *fat32_cache_find_block(fat32_cache_manager_t *cache_mgr, 
                                                   uint32_t cluster_num);
static fat32_cache_block_t *fat32_cache_alloc_block(fat32_cache_manager_t *cache_mgr);
static fat32_error_t fat32_cache_load_cluster(fat32_cache_block_t *block,
                                              fat32_disk_t *disk,
                                              const fat32_fs_info_t *fs_info,
                                              uint32_t cluster_num);
static fat32_error_t fat32_cache_write_back_block(fat32_cache_block_t *block,
                                                  fat32_disk_t *disk,
                                                  const fat32_fs_info_t *fs_info);
                                              ...
                                              uint32_t cluster_num)
{
    ...
    // 从磁盘读取数据
    fat32_error_t result = fat32_disk_read_sectors(disk, first_sector,
                                                   fs_info->sectors_per_cluster, block->data);
    if (result != FAT32_OK) {
        return result;
    }
                                                  ...
                                                  const fat32_fs_info_t *fs_info)
{
    ...
    // 写入磁盘
    return fat32_disk_write_sectors(disk, first_sector,
                                    fs_info->sectors_per_cluster, block->data);
}
