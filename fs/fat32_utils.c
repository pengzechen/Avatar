 ...
 *
 * @file fat32_utils.c
 * @brief Implementation of fat32_utils.c
...
fat32_utils_fat_to_datetime(uint16_t fat_time, uint16_t fat_date, fat32_datetime_t *datetime)
{
    ...
    // 基本有效性检查
    if (datetime->month == 0 || datetime->month > 12 || datetime->day == 0 || datetime->day > 31 ||
                            ...
                            uint16_t               *fat_date)
{
    ...
    // 基本有效性检查
    if (datetime->year < 1980 || datetime->year > 2107 || datetime->month == 0 ||
        datetime->month > 12 || datetime->day == 0 || datetime->day > 31 || datetime->hour > 23 ||
        datetime->minute > 59 || datetime->second > 59) {
        return FAT32_ERROR_INVALID_PARAM;
    }

    // 构造FAT32日期格式
    *fat_date = ((datetime->year - 1980) << 9) | (datetime->month << 5) | datetime->day;

    // 构造FAT32时间格式（秒除以2）
    *fat_time = (datetime->hour << 11) | (datetime->minute << 5) | (datetime->second / 2);
...
fat32_utils_print_file_attributes(uint8_t attr)
{
    ...
    if (attr & FAT32_ATTR_READ_ONLY)
        logger("R");
    if (attr & FAT32_ATTR_HIDDEN)
        logger("H");
    if (attr & FAT32_ATTR_SYSTEM)
        logger("S");
    if (attr & FAT32_ATTR_VOLUME_ID)
        logger("V");
    if (attr & FAT32_ATTR_DIRECTORY)
        logger("D");
    if (attr & FAT32_ATTR_ARCHIVE)
        logger("A");
    logger(")\n");
}
