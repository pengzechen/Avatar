...
static uint64_t pt0[PAGE_TABLE_ENTRIES]
    __attribute__((aligned(PAGE_TABLE_ALIGNMENT)));  // 低地址空间L0页表
static uint64_t pt1[PAGE_TABLE_ENTRIES]
    __attribute__((aligned(PAGE_TABLE_ALIGNMENT)));  // 低地址空间L1页表

static uint64_t high_pt0[PAGE_TABLE_ENTRIES]
    __attribute__((aligned(PAGE_TABLE_ALIGNMENT)));  // 高地址空间L0页表
static uint64_t high_pt1[PAGE_TABLE_ENTRIES]
    __attribute__((aligned(PAGE_TABLE_ALIGNMENT)));  // 高地址空间L1页表
...
/* 外部汇编函数声明 */
extern void
init_mmu(uint64_t ttbr0, uint64_t ttbr1);
extern void
init_mmu_el2(uint64_t ttbr0);
...
validate_memory_layout(void)
{
    ...
    // 验证每个区域都是1GB对齐的
    if (!IS_1GB_ALIGNED(DEVICE_MEMORY_BASE) || !IS_1GB_ALIGNED(NORMAL_MEMORY_BASE) ||
        !IS_1GB_ALIGNED(USER_MEMORY_BASE) || !IS_1GB_ALIGNED(USER_MEMORY_EXT_BASE)) {
        return false;
    }
...
init_page_table(void)
{
    ...
    set_block_entry(&pt1[2],
                    USER_MEMORY_BASE,
                    PTE_NORMAL_MEMORY,
                    _AP_EL0 | _PXN);  // EL0用户内存 (禁止特权执行)
    set_block_entry(&pt1[3],
                    USER_MEMORY_EXT_BASE,
                    PTE_NORMAL_MEMORY,
                    _AP_EL0 | _PXN);  // EL0用户内存 (禁止特权执行)
