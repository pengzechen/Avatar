...
vtimer_t *alloc_vtimer(void)
{
    ...
    logger_info("Allocated vtimer %d, start_time=0x%llx, cntvoff=0x%llx\n",
                _vtimer_num - 1, vtimer->start_time, vtimer->cntvoff);
    return vtimer;
}
...
void vtimer_write_cntv_tval(tcb_t *task, int32_t tval)
{
    ...
    logger_vtimer_debug("vCPU %d: TVAL write %d, CVAL=%llu, now=%llu\n",
                vt->id, tval, new_cval, current_time);
}
...
void vtimer_core_save(tcb_t *task)
{
    ...
    if (vt->pending && vt->last_fire_time > 0) {
        ...
        if (time_since_fire > 6250000) {
            logger_warn("vCPU %d: Force clearing pending after %llu ticks (cval=%llu, now=%llu, ctl=0x%x)\n",
                        vt->id, time_since_fire, cval, current_virtual_time, ctl);
            vt->pending = false;
            // 也清除我们设置的 ISTATUS 位
            vt->cntv_ctl &= ~CNTV_CTL_ISTATUS;
...
bool vtimer_should_fire(vtimer_core_state_t *vt, uint64_t now)
{
    ...
    if (should_fire) {
        logger_vtimer_debug("vCPU %d: FIRE CHECK - now=%llu, cval=%llu, diff=%lld\n",
                    vt->id, now, vt->cntv_cval, (int64_t)now - (int64_t)vt->cntv_cval);
    }
    ...
    for (uint32_t vm_idx = 0; vm_idx < _vtimer_num; vm_idx++) {
        ...
        for (uint32_t vcpu_idx = 0; vcpu_idx < vtimer->vcpu_cnt; vcpu_idx++) {
            ...
            if (should_fire && !vt->pending) {
                logger_vtimer_debug("[pcpu: %d]: Timer fired for VM%d vCPU %d(task: %d) - guest_cval=%llu, current_time=%llu, diff=%lld\n",
                    get_current_cpu_id(), task->curr_vm->vm_id, get_vcpuid(task), task->task_id,
                    guest_cval, current_time, (int64_t)current_time - (int64_t)guest_cval);
