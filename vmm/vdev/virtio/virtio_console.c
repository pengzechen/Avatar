 ...
 * @brief Implementation of virtio_console.c
...
// 前向声明
static void
console_queue_notify(virtio_device_t *dev, uint32_t queue_idx);
static void
console_reset(virtio_device_t *dev);
...
virtio_console_create(uint64_t base_addr, uint32_t irq)
{
    ...
    // 设置设备特性
    console->dev->device_features =
        (1ULL << VIRTIO_CONSOLE_F_SIZE) | (1ULL << VIRTIO_CONSOLE_F_EMERG_WRITE);
...
void
virtio_console_destroy(virtio_console_t *console)
{
    if (!console)
        return;
...
// 会调用 vgic inject
static void
console_handle_tx_queue(virtio_console_t *console)
{
    if (!console || !console->dev)
        return;
            ...
            if (!(desc->flags & VIRTQ_DESC_F_WRITE)) {
                ...
                for (uint32_t j = 0; j < len; j++) {
                    if (data[j] != 0) {
                        ...
                        // 简化实现：存储到输出缓冲区
                        uint32_t next_tail =
                            (console->output_tail + 1) % sizeof(console->output_buffer);
...
// 会调用 vgic inject
static void
console_handle_rx_queue(virtio_console_t *console)
{
    if (!console || !console->dev)
        return;
...
void
virtio_console_input(virtio_console_t *console, const char *data, uint32_t len)
{
    if (!console || !data)
        return;
...
uint32_t
virtio_console_output(virtio_console_t *console, char *buffer, uint32_t max_len)
{
    if (!console || !buffer)
        return 0;
