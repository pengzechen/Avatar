...
// 解析寄存器访问参数
static inline reg_access_params_t parse_reg_access(stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
...
// GICD_CTLR 寄存器写操作处理
static void handle_gicd_ctlr_write(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    vgic->gicd_ctlr = params.value;
    logger_vgic_debug("GICD_CTLR write: gpa=0x%llx, value=0x%llx, len=%d\n", gpa, params.value, params.len);
}

// GICD_CTLR 寄存器读操作处理
static void handle_gicd_ctlr_read(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
...
static void handle_gicd_isenabler0_write(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    ...
    logger_vgic_debug("GICD_ISENABLER(0) write: gpa=0x%llx, value=0x%llx, len=%d, enabled_mask=0x%x\n",
                      gpa, params.value, params.len, vgicc->sgi_ppi_isenabler);
}

// GICD_ISENABLER(1+) SPI 使能寄存器写操作处理
static void handle_gicd_isenabler_spi_write(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    ...
    logger_vgic_debug("GICD_ISENABLER(%d) write: gpa=0x%llx, value=0x%llx, len=%d, enabled_mask=0x%x\n",
                      reg_idx, gpa, params.value, params.len, vgic->gicd_scenabler[reg_idx]);
}

// GICD_ISENABLER(0) SGI+PPI 使能寄存器读操作处理
static void handle_gicd_isenabler0_read(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
...
static void handle_gicd_isenabler_spi_read(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    int32_t reg_idx = (gpa - GICD_ISENABLER(0)) / 0x4;
...
static void handle_gicd_icenabler0_write(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
    ...
    logger_vgic_debug("GICD_ICENABLER(0) write: gpa=0x%llx, value=0x%llx, len=%d, enabled_mask=0x%x\n",
                      gpa, params.value, params.len, vgicc->sgi_ppi_isenabler);
}

// GICD_ICENABLER(1+) SPI 禁用寄存器写操作处理
static void handle_gicd_icenabler_spi_write(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    ...
    logger_vgic_debug("GICD_ICENABLER(%d) write: gpa=0x%llx, value=0x%llx, len=%d, enabled_mask=0x%x\n",
                      reg_idx, gpa, params.value, params.len, vgic->gicd_scenabler[reg_idx]);
}

// GICD_ICENABLER(0) SGI+PPI 禁用寄存器读操作处理
static void handle_gicd_icenabler0_read(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
    uint32_t enabled_mask = vgicc->sgi_ppi_isenabler;
    vgicd_read(info, el2_ctx, &enabled_mask);
...
static void handle_gicd_icenabler_spi_read(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    int32_t reg_idx = (gpa - GICD_ICENABLER(0)) / 0x4;
    uint32_t enabled_mask = vgic->gicd_scenabler[reg_idx];
    vgicd_read(info, el2_ctx, &enabled_mask);
...
static void handle_gicd_ispender0_write(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
...
static void handle_gicd_icpender0_write(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
...
static void handle_gicd_ispender0_read(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
...
static void handle_gicd_icpender0_read(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
    uint32_t pending_status = vgic_get_sgi_ppi_pending_status(vgicc);
    vgicd_read(info, el2_ctx, &pending_status);
...
static void handle_gicd_ipriorityr_sgi_ppi_write(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
    ...
    logger_vgic_debug("GICD_IPRIORITYR(sgi-ppi) write: offset=%d, len=%d, val=0x%llx\n", offset, params.len, params.value);
}

// GICD_IPRIORITYR SPI 优先级寄存器写操作处理
static void handle_gicd_ipriorityr_spi_write(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
    int32_t offset = gpa - GICD_IPRIORITYR(0);
...
static void handle_gicd_ipriorityr_sgi_ppi_read(tcb_t *curr, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    vgic_core_state_t *vgicc = get_vgicc_by_vcpu(curr);
...
static void handle_gicd_ipriorityr_spi_read(vgic_t *vgic, stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    uint32_t word_offset = (gpa - GICD_IPRIORITYR(0)) / 4;
...
static void handle_gicd_sgir_write(stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    reg_access_params_t params = parse_reg_access(info, el2_ctx);
...
// 处理 GICD 读操作
static void handle_gicd_read_operations(tcb_t *curr, struct _vm_t *vm, vgic_t *vgic,
                                        stage2_fault_info_t *info, trap_frame_t *el2_ctx, paddr_t gpa)
{
    if (gpa == GICD_CTLR)
    {
