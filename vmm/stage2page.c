 ...
 *
 * @file stage2page.c
 * @brief Implementation of stage2page.c
...
guest_ept_init(void)
{
    ...
    for (index_l1 = 0; index_l1 < LPAE_L1_SIZE; index_l1++) {
        ...
        for (index_l2 = 0; index_l2 < LPAE_ENTRIES; index_l2++) {
            lpae_t  entry_l2;
            lpae_t *ept_l3 =
                &ept_L3_root[LPAE_ENTRIES * LPAE_ENTRIES * index_l1 + LPAE_ENTRIES * index_l2];
            ...
            for (index_l3 = 0; index_l3 < LPAE_ENTRIES; index_l3++) {
                ...
                if (isInMemory(gpa)) {
                    ...
                    entry_l3.p2m.sh = 0x03;
                    entry_l3.p2m.mattr =
                        0xF; /* 1111b: Outer Write-back Cacheable / Inner write-back cacheable */
                } else {
                    /* Device area */
...
int32_t
gva_to_ipa(uint64_t va, uint64_t *paddr)
{
...
void
data_abort_handler(stage2_fault_info_t *info, trap_frame_t *el2_ctx)
{
    lpae_t       *ept;
    unsigned long tmp;
