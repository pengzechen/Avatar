 ...
 *
 * @file kallocator.c
 * @brief Implementation of kallocator.c
...
// 前向声明
static void
kallocator_coalesce_free_blocks(void);
static int
kallocator_add_page(void    *page_addr,
                    uint32_t page_count,
                    bool     is_large_alloc,
                    uint32_t actual_used_size);
static void
kallocator_remove_page(void *page_addr);
static page_info_t *
kallocator_find_page(void *addr);
static bool
kallocator_is_page_empty(page_info_t *page);
static void *
kalloc_small_block(uint32_t size, uint32_t alignment);

int
kallocator_init(void)
{
...
void *
kalloc(uint32_t size, uint32_t alignment)
{
    if (!g_kallocator_initialized) {
...
void *
kalloc_simple(uint32_t size)
{
    return kalloc(size, 0);  // 使用默认对齐
}

void *
kalloc_aligned(uint32_t size, uint32_t alignment)
{
...
uint64_t
kallocator_get_used_memory(void)
{
    if (!g_kallocator_initialized) {
        return 0;
    }
...
uint64_t
kallocator_get_free_memory(void)
{
    if (!g_kallocator_initialized) {
        return 0;
    }
...
bool
kallocator_is_initialized(void)
{
    return g_kallocator_initialized;
}

void
kallocator_info(void)
{
    if (!g_kallocator_initialized) {
        logger_error("Kernel allocator not initialized\n");
        return;
    ...
    logger_info("  Total allocated: %lu bytes (%lu KB)\n",
                g_kallocator.total_size,
                g_kallocator.total_size / 1024);
...
// 新的API函数
uint32_t
kallocator_get_page_count(void)
{
    if (!g_kallocator_initialized) {
        return 0;
    }
...
uint32_t
kallocator_get_small_alloc_count(void)
{
    if (!g_kallocator_initialized) {
        return 0;
    }
...
uint32_t
kallocator_get_large_alloc_count(void)
{
    if (!g_kallocator_initialized) {
        return 0;
    }
...
kallocator_coalesce_free_blocks(void)
{
    ...
    while (merged && merge_iterations < MAX_MERGE_ITERATIONS) {
        ...
        while (current != NULL && current->next != NULL) {
            ...
            // 验证块的有效性
            if (current->size == 0 || current->size > PAGE_SIZE || next->size == 0 ||
                next->size > PAGE_SIZE) {
